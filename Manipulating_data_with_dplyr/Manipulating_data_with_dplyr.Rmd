...
| dplyr is a fast and powerful R package written by Hadley Wickham
| and Romain Francois that provides a consistent and concise
...
| read.csv() with two arguments, path2csv and stringsAsFactors = FALSE, and save the result in a
| new variable called mydf. Check ?read.csv if you need help.
...
Error: invalid version specification ‘dplyr’
> packageVersion("dplyr")
[1] ‘1.0.2’
...
| That's correct!
  ...
  |============                                                                              |  13%
| If your dplyr version is not at least 0.4.0, then you should hit the Esc key now, reinstall
| dplyr, then resume this lesson where you left off.
...
| Nice work!
  ...
  |==================                                                                        |  20%
| This output is much more informative and compact than what we would get if we printed the
| original data frame (mydf) to the console.

...

  |====================                                                                      |  22%
| First, we are shown the class and dimensions of the dataset. Just below that, we get a preview of
| the data. Instead of attempting to print the entire dataset, dplyr just shows us the first 10
| rows of data and only as many columns as fit neatly in our console. At the bottom, we see the
| names and classes for any variables that didn't fit on our screen.
...
| Keep working like that and you'll get there!
  ...
  |=======================================                                                   |  43%
| Oops! That gaves us a vector of numbers from -5 through 20, which is not what we want. Instead, we want
| to negate the entire sequence of numbers from 5 through 20, so that we get -5, -6, -7, ... , -18, -19,
| -20. Try the same thing, except surround 5:20 with parentheses so that R knows we want it to first come
...
| == "swirl" returns a vector of TRUEs and FALSEs. filter() then returns only the rows of cran
...
| That's correct!
  ...
  |====================================================                                      |  58%
| Our last two calls to filter() requested all rows for which some condition AND another condition were
| TRUE. We can also request rows for which EITHER one condition OR another condition are TRUE. For example,
...
| You are doing so well!
  ...
  |============================================================                              |  67%
| Okay, ready to put all of this together? Use filter() to return all rows of cran for which r_version is
| NOT NA. Hint: You will need to use !is.na() as part of your second argument to filter().
...
...
  ...
  |================================================================                          |  72%
| To see how arrange() works, let's first take a subset of cran. select() all columns from size through
...
| Nice work!
  ...
  |====================================================================                      |  75%
| To do the same, but in descending order, change the second argument to desc(ip_id), where desc() stands
...
| select() to grab 3 columns from cran -- ip_id, package, and size (in that order) -- and store the result
...
| megabytes (MB) are a more common unit of measurement. One megabyte is equal to 2^20 bytes. That's 2 to
| the power of 20, which is approximately one million bytes!

...

  |==============================================================================            |  87%
| We want to add a column called size_mb that contains the download size in megabytes. Here's the code to
| do it:
| 
...
| command as above, except add a third argument creating a column that is named size_gb and equal to
| size_mb / 2^10.
...
| we're interested in knowing the average download size. summarize(cran, avg_bytes = mean(size)) will yield
| the mean value of the size variable. Here we've chosen to label the result 'avg_bytes', but we could have
...
Selection: 1
What is your email address? sachin.bhopi@rait.ac.in
What is your assignment token? RZiaiobqlTnWeP2w
